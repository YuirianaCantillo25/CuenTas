    ...
    if (!$conexion) {
        ...
        if (!$resultado_verificar) {
            ...
            if (password_verify($contraseña, $fila['contraseña'])) {
                ...
                $_SESSION['usuario_rol'] = $fila['rol']; // Obtener el rol del usuario

                // Redireccionar según el rol del usuario
                if ($fila['rol'] == 'administrador') {
                    header("Location: CuenTasAdmin.html?exito=" . urlencode("Inicio de sesión exitoso para administrador."));
                    exit();
                } elseif ($fila['rol'] == 'vendedor') {
                    header("Location: CuenTasUser.html?exito=" . urlencode("Inicio de sesión exitoso para vendedor."));
                    exit();
                }
            } else {
